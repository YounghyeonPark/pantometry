...
| `pantometry-world` | The first consumer, and not published. Worlds described as data: built, coupled over the bus, run and drawn, with thirty-one scenes across twelve of the thirteen domains that CI runs. It exists to use the SDK from outside and write down where that is awkward |
