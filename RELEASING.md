...
| `README.md` | **4 occurrences, 2 of which move** — the `pantometry = "0.x"` install line and the BibTeX block's `version`. The other two are the sentence naming whose DOI the `doi` field is, which moves only when one is minted, and a record of a past mistake that must not move at all. So `grep -c` counts four here and two is the answer; the row said 1. The BibTeX `version`, which is **not** the `doi` beside it and does not move at the same time. **The row this table was missing at 0.20.0**: the seven-file grep below does not reach it, and the 0.19.0 release only got it right because it was bumped by hand as a separate step. It shipped stale for the length of one release. Four times wrong now, in both directions. **Guarded since**: `documented_version.rs` holds the `version` against the crate, the `doi` against the table at the bottom of this file, and the `url` against `CITATION.cff`’s concept DOI — which is the one that catches a version DOI written where the concept one belongs, a mistake that differs by a single digit. Between the bump and the tag there is no row to check against, and it says so rather than failing |
...
| 0.22.0 | `10.5281/zenodo.22979609` — **thirty-six minutes after the webhook.** `published` answered 202 at 15:03:50 UTC and the record was created at 15:39:27; until then the search API and `versions/latest` on the previous record both answered 0.21.0, and every query in those minutes — fifty-four of them — found nothing. So half an hour of negatives is still *unknown* and not *failed* — which is what the section below says about a negative, and this is the measurement behind it. Deleting and recreating the release at twenty minutes, which is the remedy for a failed deposition, would have been a remedy for nothing |
