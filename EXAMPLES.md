...
| `optical_bench` | **A 3D instrument, not a graph.** A doublet **drawn as the surfaces it is**, a fold mirror turning the axis through 90°, three field angles — prescribed, traced, refocused, then *bent* until the spot falls inside the Airy disc. Every point of the drawn glass is checked against the intersection the rays use, and the shape gives a check the prescription cannot: each element’s thinnest point. `optical_bench bench.html` gives a layout you rotate in a browser |
...
| `ligand_binding` | **What a small molecule does to a protein's motions.** Adenylate kinase open and empty (`4AKE`) against the same enzyme closed on the inhibitor `AP5A` (`1AKE`), from the crystallographers' own files. The softest mode of the *open* structure is the closing motion it is observed to perform — overlap `0.799` against `0.039` for a direction that knows nothing, and the model never sees the closed one. Then what binding does: a ligand adds **nodes**, so the complex has three modes more per ligand atom and its spectrum is a different problem's; the protein's own coordinates are the Schur complement, and not one of the 214 residues moves more once the ligand is there. Writes a figure with a name ending `.svg`, and with `.json` the closing itself: 48 frames of the backbone as a swept solid over the molecule, each frame's mesh checked closed, right-side-out and not folded through itself |
