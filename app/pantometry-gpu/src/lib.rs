...
pub struct GpuSolid {
    ...
    /// The block this mirrors, kept for one thing: [`ScalarField::at`].
    ///
    /// Sampling a grid is trilinear interpolation, clamping at the faces, and — the part that is
    /// easy to get wrong — void handling, where "a sample belongs to the cell it is in, and if that
    /// cell is empty there is nothing there to sample" took a measurement to get right. A second
    /// copy of that here would be a second chance at it, so `Solid3D::sample_with` does it with
    /// this domain's values.
    cpu: Solid3D,
    /// The device's cells, read back. `RefCell` because the trait methods that need them are
    /// `&self` — see [`GpuSolid::sync`].
    mirror: std::cell::RefCell<Vec<f32>>,
    mirror_valid: std::cell::Cell<bool>,
    /// `mirror` as `f64` kelvin, for `sample_with`. Rebuilt on each sync rather than on each
    /// sample: a panel asks for one value per cell and would otherwise convert the grid per point.
    kelvin: std::cell::RefCell<Vec<f64>>,
}
...
impl GpuSolid {
    ...
    pub fn mirroring(cpu: Solid3D) -> Result<GpuSolid, Unavailable> {
        ...
        let kelvin = mirror.iter().map(|v| reference + *v as f64).collect();
        let name = pantometry_core::Domain::name(&cpu).to_string();
        Ok(GpuSolid {
            cpu,
            kelvin: std::cell::RefCell::new(kelvin),
            name,
            counts,
            dx,
            alpha,
            ...
            mirror_valid: std::cell::Cell::new(true),
        })
    }
    ...
    pub fn temperature_at(&mut self, i: usize, j: usize, k: usize) -> Temperature {
        ...
        Temperature::from_si(self.reference + self.current()[idx] as f64)
    }
    ...
    pub fn set_temperature(&mut self, i: usize, j: usize, k: usize, t: Temperature) {
        ...
        self.sync();
        self.mirror.get_mut()[i + nx * (j + ny * k)] = (t.to_si() - self.reference) as f32;
        self.gpu.upload(self.mirror.get_mut());
        self.mirror_valid.set(false);
    }
    ...
    pub fn deposit(
        ...
        let idx = i + nx * (j + ny * k);
        self.sync();
        self.mirror.get_mut()[idx] += (joules.to_si() / self.capacity[idx]) as f32;
    ...
    ///
    /// A copy rather than a borrow: the mirror lives behind a `RefCell` — because the trait methods
    /// that read it are `&self` and nothing hands this domain a `&mut` during a run — and handing
    /// out a reference into it would tie a caller's lifetime to a runtime borrow it cannot see.
    ...
    pub fn mean_temperature(&mut self) -> Temperature {
        ...
        Temperature::from_si(self.reference + cells.iter().map(|v| *v as f64).sum::<f64>() / n)
    }
    ...
    pub fn peak_temperature(&mut self) -> Temperature {
        self.sync();
        Temperature::from_si(
            self.reference + self.current().iter().fold(f32::MIN, |m, v| m.max(*v)) as f64,
        )
    }

    /// Bring the mirror up to date, from `&self`.
    ///
    /// **This has to work without `&mut`, and that is the whole reason for the `RefCell`.** The
    /// trait methods that read a domain's state — `ledger`, `readings`, `ScalarField::at` — all take
    /// `&self`, and `Simulation::advance` calls `ledger` before and after each advance without ever
    /// handing back a `&mut`. So the mirror was never refreshed during a run: measured, a scene run
    /// on the device reported `peak = 120.0000011 °C` at **every frame** while the CPU's spot
    /// diffused 120 → 23.4 → 21.3 → 20.7. The audit read the same frozen ledger, which is an audit
    /// that cannot fail.
    ///
    /// One readback per advance, not per substep: `advance` reads the ledger twice and the flag is
    /// cleared once per `step`, so a multirate domain taking twenty substeps still transfers once
    /// when something finally asks.
    fn sync(&self) {
        if !self.mirror_valid.get() {
            self.gpu.download(&mut self.mirror.borrow_mut());
            self.mirror_valid.set(true);
            let reference = self.reference;
            let mirror = self.mirror.borrow();
            let mut kelvin = self.kelvin.borrow_mut();
            kelvin.clear();
            kelvin.extend(mirror.iter().map(|v| reference + *v as f64));
        }
    }

    /// The device's cells, current, as deviations from the reference.
    fn current(&self) -> std::cell::Ref<'_, Vec<f32>> {
        self.sync();
        self.mirror.borrow()
    }

    /// The same, as absolute kelvin, for [`Solid3D::sample_with`].
    fn current_kelvin(&self) -> std::cell::Ref<'_, Vec<f64>> {
        self.sync();
        self.kelvin.borrow()
    }
...
impl Domain for GpuSolid {
    ...
    fn step(&mut self, _t: Time, dt: Time, bus: &mut Exchange) -> Result<(), Violation> {
        ...
        if gained != 0.0 {
            ...
            self.sync();
            for cell in self.mirror.get_mut().iter_mut() {
                *cell += per_cell;
            }
            self.gpu.upload(self.mirror.get_mut());
        }
    ...
    /// **Two contributions, not their sum**, for the reason `Solid3D::ledger` states in full.
    ///
    /// `Ledger` raises an entry's *scale* to the largest single contribution, and the audit judges a
    /// change against that scale. This recorded one entry — the net stored heat — and the buffer
    /// holds deviations from the block's **mean**, so that net is `Σ cᵢ(Tᵢ − T̄)`, which is
    /// identically zero when the capacities are equal. A relative tolerance against zero is not a
    /// tolerance, and it fired: measured, `6.77e-9 J` became `-1.39e-8 J` and was reported as a
    /// 148% change on the first step of a correct run.
    ///
    /// Splitting the sum by sign leaves the net untouched and makes the scale the heat that is
    /// actually distributed — 0.242 J for one cell 100 K above its neighbours, rather than the
    /// nanojoule the two halves cancel to. The CPU domain learned the same thing from a source
    /// opening its books at exactly zero.
    fn ledger(&self) -> Ledger {
        ...
        let (up, down) =
            self.capacity
                .iter()
                .zip(cells.iter())
                .fold((0.0f64, 0.0f64), |(up, down), (c, d)| {
                    let e = c * *d as f64;
                    if e >= 0.0 {
                        (up + e, down)
                    } else {
        ...
        Ledger::new()
            .with(quantity::ENERGY, up)
            .with(quantity::ENERGY, down)
    }
    ...
    fn readings(&self) -> Vec<Reading> {
        ...
        let mean = self.reference + cells.iter().map(|v| *v as f64).sum::<f64>() / n;
        let peak = self.reference + cells.iter().fold(f32::MIN, |m, v| m.max(*v)) as f64;
        let coldest = self.reference + cells.iter().fold(f32::MAX, |m, v| m.min(*v)) as f64;
        vec![
    ...
    /// **The field the run draws.** Without this a scene run on the device had no picture at all.
    ///
    /// `capture` asks each domain for one through `as_field`, and a `None` there means no panel —
    /// so the report drew nothing, the CSV had only scalars, and glTF, USD and the viewer had no
    /// geometry to carry. The CLI said "no field and no bodies — not drawn" about a block, which
    /// is true of what the domain *offered* and absurd about a block.
    ///
    /// Found by running one scene both ways, which is a thing no binary could do until the CLI, the
    /// viewer, the editor and this crate were one application.
...
impl pantometry_core::ScalarField for GpuSolid {
    ...
    /// The block's own geometry, with the device's values.
    ///
    /// [`Solid3D::sample_with`] does the interpolation, the clamping and the void handling. A second
    /// copy here would be a second chance at the rule that took a measurement to get right — "a
    /// sample belongs to the cell it is in, and if that cell is empty there is nothing there to
    /// sample" — and the two pictures would have to agree for the CPU to be the reference at all.
    fn at(&self, p: pantometry_core::units::LengthVec, t: Time) -> f64 {
        self.cpu.sample_with(p, t, &self.current_kelvin())
    }
}
