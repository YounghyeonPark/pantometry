...
fn the_first_sample_is_the_low_corner_and_the_last_is_the_high_one() {
    ...
    let out = field_points(&corners, (nx, ny, nz), Lattice::Nodal, &values, 1);
...
fn a_single_sample_along_an_axis_is_at_its_middle() {
    let out = field_points(&unit_box(), (1, 1, 1), Lattice::Nodal, &[7.0], 1);
    assert_eq!(out.len(), 1);
...
fn a_hole_is_not_a_point() {
    ...
    let out = field_points(&corners, (2, 2, 2), Lattice::Nodal, &values, 1);
...
fn a_stride_thins_each_axis_and_never_takes_nothing() {
    ...
    assert_eq!(
        field_points(&corners, (nx, ny, nz), Lattice::Nodal, &values, 1).len(),
        125
    );
    // 0, 2, 4 along each axis.
    assert_eq!(
        field_points(&corners, (nx, ny, nz), Lattice::Nodal, &values, 2).len(),
        27
    );
    // A stride of zero would step by nothing and never finish; it is read as one.
    assert_eq!(
        field_points(&corners, (nx, ny, nz), Lattice::Nodal, &values, 0).len(),
        125
    );
}
...
fn a_short_value_array_draws_nothing() {
    let corners = unit_box();
    assert!(field_points(&corners, (2, 2, 2), Lattice::Nodal, &[1.0, 2.0], 1).is_empty());
    assert!(field_points(&corners, (0, 2, 2), Lattice::Nodal, &[1.0; 4], 1).is_empty());
}
...
fn the_samples_follow_the_box_they_are_given() {
    ...
    let out = field_points(&corners, (2, 1, 1), Lattice::Nodal, &[1.0, 2.0], 1);
    assert_eq!(out.len(), 2);
