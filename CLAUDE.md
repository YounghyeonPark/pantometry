...
| [EVIDENCE.md](EVIDENCE.md) | claiming that something is checked. Fourteen sections of what the closed forms are and what checking against them found, including two defects the conservation audit called clean, one a closed form agreed with to `1.1e-4` while it was 32.5% wrong, and the only rate here that is not an integer |
...
| [app/pantometry-world/FRICTION.md](app/pantometry-world/FRICTION.md) | changing the public API. Forty-three findings from using the SDK as a stranger, five of them the same underlying decision |
